"""Simplified FastAPI Server for browser-use - minimal dependencies version.

This server provides browser automation capabilities via REST API endpoints
without requiring the full browser-use configuration system.

Usage:
    uvicorn browser_use.api.simple_server:app --host 0.0.0.0 --port 8000
"""
...
# Import only essential browser_use modules
...
def create_app() -> FastAPI:
    ...
    async def create_session(request: CreateSessionRequest):
        ...
        try:
            ...
            file_system_path = '~/.browser-use-api'
...
def main():
    ...
    uvicorn.run(
        "browser_use.api.simple_server:app",
        host=args.host,
        port=args.port,
        reload=args.reload
    )
